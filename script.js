...
const BACKEND_URL = "https://zomechat-motor.onrender.com/"; // Örnektir, kendi adresinle değiştir.
...
// Firebase Giriş Durumunu Sürekli Dinleyen Mekanizma
if (typeof auth !== 'undefined') {
    auth.onAuthStateChanged((user) => {
        if (user) {
            console.log("Kullanıcı başarıyla giriş yaptı:", user.displayName);
            window.startAppWithUser(user.displayName);
        } else {
            console.log("Aktif kullanıcı oturumu yok.");
            window.showLoginScreen();
        }
    });
} else {
    console.error("Firebase Auth kütüphanesi bulunamadı! Lütfen index.html dosyanızı kontrol edin.");
}